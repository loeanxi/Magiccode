...
package com.magiccode.command;

import com.magiccode.command.Command.CommandType;
...
/**
 * 从 .magiccode/commands/ 目录加载自定义 Markdown 命令文件。
...
public final class CommandLoader {
    ...
    /**
     * 合并用户全局和项目级的文件命令。
     * 搜索路径：1. ~/.magiccode/commands/  2. $workDir/.magiccode/commands/
     * 后者覆盖前者同名命令。
     */
    public static List<Command> loadUserCommands(String workDir) {
        ...
        if (home != null && !home.isEmpty()) {
            dirs.add(Path.of(home, ".magiccode", "commands").toString());
        }
        dirs.add(Path.of(workDir, ".magiccode", "commands").toString());

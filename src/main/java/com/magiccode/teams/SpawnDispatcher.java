...
public final class SpawnDispatcher {
    ...
    public record SpawnConfig(
            ...
            String protocol,
            com.magiccode.config.ProviderConfig providerConfig,
            String workdir
    ) {}
     ...
     * Format: cd '<workdir>' && '<magiccode>' --teammate --team-name <t> --agent-name <n>
     */
    public static String buildTeammateCLI(String teamName, String memberName, String workdir) {
        ...
        String magiccode = ProcessHandle.current().info().command().orElse("magiccode");
        return "cd %s && %s --teammate --team-name %s --agent-name %s".formatted(
                shellQuote(wd), shellQuote(magiccode), shellQuote(teamName), shellQuote(memberName));
    }

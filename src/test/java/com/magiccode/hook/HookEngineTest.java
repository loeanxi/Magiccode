...
class HookEngineTest {
    ...
    // ---- MAGICCODE_FILE_PATH 环境变量测试 ----

    @Test
    void testCommandInjectsFilePathEnvVar() {
        // 跳过 bash 不可用的环境（如部分 Windows 环境缺少 echo 等内置命令）
        assumeTrue(isBashFunctional(), "Skipping: bash is not functional on this system");

        var engine = new HookEngine();
        engine.loadHooks(List.of(new Hook(
                "echo-path", EventName.POST_TOOL_USE, null,
                new Action(ActionType.COMMAND, "echo $MAGICCODE_FILE_PATH", null),
                false)));
    ...
    /**
     * 检测当前系统的 bash 是否能正常执行命令。
     * 某些 Windows 环境下 bash 存在但内置命令（echo 等）不可用。
     */
    private static boolean isBashFunctional() {
        try {
